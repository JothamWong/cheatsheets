...
\begin{multicols*}{4}
  ...
  For each input action (R, W, C, A) to the scheduler, 
  \begin{itemize}
    \item output action to S
    \item Postpone action by blocking Xact
    \item Reject action and abort Xact
  \end{itemize}
  ...
  \subsection{Two Phase Locking (2PL) Protocol}
  \begin{enumerate}
  ...
  \begin{itemize}
    \item Older T have higher priority than younger T
    \begin{itemize}
      \item Each T is assigned timestamp when started. 
      \item Older T has a smaller timestamp.
    \end{itemize}
    \item Suppose $T_i$ requests for a lock that conflicts with a lock held by $T_j$
    \item \includegraphics[width=0.19\textwidth]{cs3223-deadlock-prevention.png}
  ...
  \subsection{Lock Conversion}
  Increases concurrency by allowing lock conversions, previously serial only schedules can become interleaved.
  \begin{itemize}
  ...
  \subsection{Lock-based Isolation levels}
  \includegraphics[width=0.20\textwidth]{cs3223-isolation-level.png}
  \begin{itemize}
  ...
  \begin{itemize}
    \item Highest(coarsest) to lowest(finest): db, relation, page, tuple
